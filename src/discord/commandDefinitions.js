...
const levelroleCommand = new SlashCommandBuilder()
  ...
  .addSubcommand((s) => s.setName('set').setDescription('Definit un palier')
    ...
    .addRoleOption((o) => o.setName('role').setDescription('Role a attribuer').setRequired(false))
    .addIntegerOption((o) => o.setName('bonus').setDescription('Bonus economie (piece) verse a ce palier').setRequired(false).setMinValue(1))
    .addStringOption((o) => o.setName('annonce').setDescription('Message custom ({user} et {level} remplaces) — sinon message par defaut').setRequired(false).setMaxLength(200)))
